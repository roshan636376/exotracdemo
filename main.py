...
def main():
    ...
    print_banner()
    parser = argparse.ArgumentParser(description='Exotrac AI Chatbot')
    parser.add_argument('--local', action='store_true', help='Run in local/mock mode without OpenAI API key')
    args = parser.parse_args()

    # Check for OpenAI API key (allow --local to bypass for development)
    ...
    if not api_key and not args.local:
        ...
        print("3. Set it in your environment variable 'OPENAI_API_KEY'")
        print("\nAlternatively, run with --local for development (no OpenAI calls).")
        print("\nExiting...")
        sys.exit(1)
    ...
    try:
        ...
        bot = ExotracChatbot(local=args.local)
        ...
        print(
            "It will automatically generate emails based on prospect interest levels."
        )
        print_help()
        ...
        while True:
            try:
                ...
                if result['should_send_email'] and result['email_type']:
                    ...
                    if result.get('next_stage') and result[
                            'next_stage'] != result['current_stage']:
                        print(
                            f"   → Advancing to Stage: {result['next_stage']}/3"
                        )
                    ...
                    if email:
                        ...
                        print(
                            f"│ To: {email['to_email'] or 'Prospect email needed'}"
                        )
                        ...
                        if result['current_stage'] == 3:
                            ...
                            company_result = email_sender.send_to_main_company(
                                result['prospect_info'])
                            if company_result['success']:
                                print(f"✅ {company_result['message']}")
                                if 'filename' in company_result:
                                    print(
                                        f"   📄 File: {company_result['filename']}"
                                    )

                # Show conversation summary periodically
                if len(bot.conversation_history) % 10 == 0 and len(
        ...
        print("2. training_dataset.json exists")
